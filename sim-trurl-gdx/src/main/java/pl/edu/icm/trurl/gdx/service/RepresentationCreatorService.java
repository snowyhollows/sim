...
public class RepresentationCreatorService {

    @WithFactory
    public RepresentationCreatorService(EngineBuilder engineBuilder) {
    ...
    public RepresentationCreator using(Session session) {
        return new RepresentationCreator(session);
    }

    public final class RepresentationCreator {
        private final Session session;

        public RepresentationCreator(Session session) {
        ...
        public void createTexturesFromAtlas(String atlasPath) {
            ...
            data.getPages().forEach(page -> {
                String name = page.textureFile.name();
                Entity textureEntity = createTextureEntity(name);
                textures.put(name, textureEntity);
            });
        ...
        private Entity createTextureRegionEntity(String name, Map<String, Entity> textures, TextureAtlas.TextureAtlasData.Region region) {
            ...
            textureRegionComponent.setTexture(textures.get(region.page.textureFile.name()));
